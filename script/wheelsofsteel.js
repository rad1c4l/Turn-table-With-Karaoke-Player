...
(function (window) {
 ...
 timer = (function () {
  ...
  requestAnimationFrame = (function () {
   return (USE_REQUEST_ANIMATION_FRAME ? (window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || null) : null);
  }());
 ...
 features.transform.prop = (features.transform.moz || features.transform.webkit || features.transform.ie || features.transform.opera || features.transform.w3);
 ...
 function Turntable(oTT, sURL) {
  ...
  this.dom = {
   ...
   canvas: null,
   cdgPlayer: null,
   playbtn: document.getElementById('vid-' + self.id.substr(self.id.length - 1))
  };
  this.dom.playbtn.setAttribute('disabled', true);
  ...
  this.loadWaveform = function (trackName, soundcloudID, soundcloudURL) {
   ...
   var fgcolor = '336699', // color to draw the waveform with
    ...
    waveformURL = 'utils/waveform/?' + ['track=' + trackName, 'width=' + self.data.waveform.sizeLimit, // several minutes will fit within the 64K-pixel limit; longer tracks will start to get scaled down.
     'height=' + defaultHeight, 'fgcolor=' + fgcolor
    ].join('&'),
    compactWaveformURL = 'utils/waveform/?' + ['track=' + trackName, 'width=' + defaultWidth, 'height=' + compactHeight, 'fgcolor=' + fgcolor].join('&');
   ...
   if (!window.location.protocol.match(/http/i)) {
    ...
    compactWaveformURL = 'utils/waveform/cache/' + [
     trackName.replace(' ', '_'), (self.data.waveform.offsetWidth + 2) + 'x' + compactHeight, // +2 for padding/border stuff, thus 498 becomes 500.
     'transparent',
     fgcolor
    ].join('_') + '.png';
   }
  ...
  this.soundEvents = {
   ...
   whileplaying: function () {
    // console.log('sound id '+this.sID+' / position: '+this.position+'/'+this.duration);x
   }
  };
  ...
  this.syncVideo = function () {
   ...
   let d = (self.data.sound.soundObject.position != null ? self.data.sound.soundObject.position : 100.124154678);
   self.dom.cdgPlayer.sync(d); // convert to ms
  }
  ...
  this.showVideo = function () {
   wheelsofsteel.video = self.id.substr(self.id.length - 1);
   $('#cdg-app').append(self.dom.canvas);
   if (!($('.modal.in').length)) {
    $('.modal-dialog').css({
     top: 0,
  ...
  this.hideVideo = function () {
   $('#myModal').modal('hide');
   let cApp = document.getElementById('cdg-app');
   while (cApp.firstChild) {
    cApp.removeChild(cApp.firstChild);
   }
   self.stopVideo();
  }
  this.hasOpenVideo = function () {
   return (self.modalShown() && (wheelsofsteel.video == self.id.substr(self.id.length - 1))) ? true : false;
  }
  this.modalShown = function () {
   return (document.getElementById('myModal').style.display == "none") ? false : true;
  }
  this.dom.playbtn.addEventListener('click', function () {
   // If the modal is already shown
   if (self.modalShown() && (wheelsofsteel.video === self.id.substr(self.id.length - 1))) {
    return;
   }
   if (wheelsofsteel.video == null) {
    self.showVideo();
    if (self.data.sound.soundObject.playState > 0) {
     self.playVideo();
    } else {
   ...
   } else if (wheelsofsteel.video !== self.id.substr(self.id.length - 1)) {
    // Close Modal
    // debugger;
    wheelsofsteel.turntables[wheelsofsteel.video - 1].hideVideo();
    wheelsofsteel.video = null;
    self.hideVideo();
    let temp = $(this);
    setTimeout(function () {
     temp.click();
 ...
 soundManager.onready(function () {
  ...
  loadURL = function (sURL, i, c, soundcloudID) {
   ...
   if (c) {
    fetch('music/0007fe4dfc0ddd92002200be94470ee0e808cbcb.cdg').then(response => response.arrayBuffer()).then(buffer => {
     turntable.data.cdgBuffer = buffer;
     turntable.dom.canvas = document.createElement('canvas');
     turntable.dom.cdgPlayer = new CDGPlayer(turntable.dom.canvas);
     turntable.dom.canvas.width = 600;
     turntable.dom.canvas.height = 432;
     turntable.dom.playbtn.removeAttribute('disabled');
     // console.log('CDG Player created.');
    });
   } else {
    if (turntable.data.cdgBuffer) {
     delete turntable.data.cdgBuffer;
     turntable.data.cdgBuffer = null;
     delete turntable.dom.canvas;
     turntable.dom.canvas = null;
     turntable.dom.playbtn.setAttribute('disabled', true);
     // console.log('CDG Player Deleted.');
    }
   }
  ...
  o.onmousedown = function (e) {
   ...
   if (target.nodeName.toLowerCase() === 'a' && !utils.hasClass(target, 'exclude')) {
    ...
    if (!track_id) {
     // debugger;
     // regular MP3
     let q = target.parentElement.parentElement;
     loadURL(`music/${target.id}.mp3`, q.id - 1 /* (e.button === 2 || e.shiftKey || e.ctrlKey || e.altKey || e.metaKey ? 1 : 0) */ , target.name == 'karaoke' ? true : false); // left vs. right-click
     if (e.preventDefault) {
      e.preventDefault();
     }
    } else {
